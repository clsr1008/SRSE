...
from sklearn.metrics import classification_report, accuracy_score, f1_score, mean_absolute_percentage_error
...
def starting_logs(data_type, da_method, exp_log_dir, scenarios, run_id):
    batches = scenarios
    log_dir = os.path.join(exp_log_dir, "_".join(batches) + "_run_" + str(run_id))
...
def save_checkpoint(home_path, algorithm, selected_scenarios, dataset_configs, log_dir, hparams):
    save_dict = {
        ...
        # "network_dict": algorithm.joint_network.state_dict(),
        # "discriminator": algorithm.domain_classifier.state_dict()
    }
...
def _calc_metrics(pred_labels, true_labels, log_dir, home_path):
    ...
    # 分别计算每个任务的RMSE MAPE r2
    for task_idx in range(len(pred_labels)):
        task_pred = np.array(pred_labels[task_idx])
        task_true = np.array(true_labels[task_idx])

        task_rmse = np.sqrt(mean_squared_error(task_true, task_pred))
        task_mape = mean_absolute_percentage_error(task_true, task_pred)
        task_r2 = r2_score(task_true, task_pred)

        task_RMSEs.append(task_rmse)
        task_MAPEs.append(task_mape)
        task_R2s.append(task_r2)

        # 汇总所有任务的标签，用于计算总体指标
        overall_true_labels.extend(task_true)
        overall_pred_labels.extend(task_pred)

    # 总体预测和标签
    overall_true_labels = np.array(overall_true_labels)
    overall_pred_labels = np.array(overall_pred_labels)

    # 生成回归报告
    regression_report_dict = regression_report(overall_true_labels, overall_pred_labels)
    print('regression_report_dict:', regression_report_dict)

    # 将报告转换为DataFrame
    regression_df = pd.DataFrame(list(regression_report_dict.items()), columns=['Metric', 'Value'])
    ...
    # 写入Excel文件
    regression_df.to_excel(report_save_path, index=False)

    return (regression_report_dict['RMSE'], regression_report_dict['MAPE'], regression_report_dict['R2'],
            task_RMSEs, task_MAPEs, task_R2s)

def copy_Files(destination):
    destination_dir = os.path.join(destination, "MODEL_BACKUP_FILES")
    os.makedirs(destination_dir, exist_ok=True)
    copy("uda_benchmark.py", os.path.join(destination_dir, "main.py"))
...
def calc_dev_risk(target_model, src_train_dl, tgt_train_dl, src_valid_dl, configs, device):
    src_train_feats = target_model.joint_learner(src_train_dl.dataset.x_data.float().to(device))
    tgt_train_feats = target_model.joint_learner(tgt_train_dl.dataset.x_data.float().to(device))
    src_valid_feats = target_model.joint_learner(src_valid_dl.dataset.x_data.float().to(device))
    src_valid_pred = target_model.joint_classifier(src_valid_feats)
...
def calculate_risk(target_model, risk_dataloader, device):
    ...
    feat = target_model.joint_learner(x_data.float().to(device))
    pred = target_model.joint_classifier(feat)
